...
const AllAsianFood = ({ asianFoods, categories }) => {
    ...
    <div className='col-span-4'>
      <div className='overflow-x-auto max-h-[500px] overflow-y-auto'>
        <table className='table w-full text-center'>
          ...
          <tbody>
            {!isEmpty(asianFoods) &&
              map(asianFoods, (asianFood) => (
                <tr key={asianFood._id}>
                  <td className='border px-8 py-4'>{asianFood.name.slice(0, 20)}</td>
                  <td className='border px-8 py-4'>{asianFood.details?.slice(0, 20)}...</td>
                  <td className='border px-8 py-4'>{asianFood.categoryId}</td>
                  <td className='border px-8 py-4'>{map(asianFood.price, (price) => price).join(', ')}</td>
                  <td className='border px-8 py-4'>
                    <Link href={`admin/asianFood/update/${asianFood._id}`}>
